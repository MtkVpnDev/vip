...
echo "deb http://ftp.debian.org/debian/ jessie main contrib non-free
    ...
    make -j$(nproc)
    make install
    wget --no-check-certificate -O /etc/init.d/squid http://admin-boyes.com/script/squid.sh
    ...
    cd /etc/squid/
    rm squid.conf
    echo "acl Tknetwork dst `curl -s https://api.ipify.org`" >> squid.conf
    echo 'http_port 8080
http_port 8181
